...
exports.exec = function(message) {
  ...
  if (klasData.klassen[klasName] && klasRole) { // check if the role was registered in the bot and in Discord
    message.channel.send("Die rol bestaat al en is al geregistreerd.");
    return;
  } // the role isn't registered in the bot or in Discord
  if (klasData.klassen[klasName] && !klasRole) { // if the role is registered in the bot but not in Discord:
    delete klasData.klassen[klasName]; // remove it from cached klas role file and carry on.
  }
  if (klasRole) { // if a discord role was found
    klasData.klassen[klasName] = {"id":klasRole.id};
    ...
    message.guild.roles.create({data:{name:klasName,permissions:0},reason:`Klas gemaakt namens ${message.author.tag}`}).then(function(newKlasRole) {
      klasData.klassen[klasName] = {"id":newKlasRole.id,"name":klasName};
      fs.writeFileSync("data/klassen.json", JSON.stringify(klasData));
